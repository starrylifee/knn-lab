...
function startLeg(room, random) {
  ...
  room.game={turn:1,phase:'choose',board:Object.fromEntries(sites.filter(s=>s.start).map(s=>[s.id,s.start])),cards:deck.splice(0,3).map((c,i)=>({...c,open:i===0,flipped:false})),deck,score:{red:0,blue:0},log:[],pending:null,flipped:null};
