...
function startLeg(room, random) {
  ...
  // 세균 비: 한 판에 2번, 3~10차례 중 서로 다른 차례에 점수 뒤진 쪽 색 1마리가 빈자리에 생긴다(점수 없음). 비 오는 차례는 비밀(view에서 제외).
  const rainTurns=shuffled([3,4,5,6,7,8,9,10],random).slice(0,2).sort((a,b)=>a-b);
  room.game={turn:1,phase:'choose',board:Object.fromEntries(sites.filter(s=>s.start).map(s=>[s.id,s.start])),cards:deck.splice(0,3).map((c,i)=>({...c,open:i===0,flipped:false})),deck,score:{red:0,blue:0},log:[],pending:null,flipped:null,rainTurns,rains:[]};
  drawCandidates(room.game,random);
}
/* 히든카드 도입 전 방: 카드에 open 표시가 없으면 첫 장을 공개 카드로 본다 */
function normalizeCards(g){if(g&&g.cards.length&&!g.cards.some(c=>c.open||c.flipped)){g.cards.forEach((c,i)=>{c.open=i===0;c.flipped=false;});g.flipped=g.flipped||null;}}
/* 차례가 시작될 때 비가 예정돼 있으면 뒤진 쪽 색 1마리를 빈자리에 놓는다. 동점이면 판 위에 적은 색, 그것도 같으면 무작위 */
function rain(r,random){
  const g=r.game;if(!g.rainTurns||!g.rainTurns.includes(g.turn))return;
  const empty=sites.filter(s=>!g.board[s.id]).map(s=>s.id);if(!empty.length)return;
  const diff=r.totals[r.redIndex]-r.totals[1-r.redIndex];
  const reds=Object.values(g.board).filter(c=>c==='red').length,blues=Object.values(g.board).length-reds;
  const color=diff<0?'red':diff>0?'blue':reds<blues?'red':blues<reds?'blue':(random(2)?'red':'blue');
  const site=empty[random(empty.length)];g.board[site]=color;g.rains.push({turn:g.turn,site,color});
}
...
function settle(r,random){
  ...
  // 쓴 카드 자리에 새 카드가 온다. 공개 카드를 썼으면 새 카드도 공개, 뒤집어 쓴 카드였으면 새 카드는 뒤집힌 채로. 공개 카드는 늘 1장 이상.
  g.cards=g.cards.filter(c=>c.id!==p.card.id);if(g.deck.length)g.cards.push({...g.deck.shift(),open:!!p.card.open,flipped:false});
  if(g.cards.length&&!g.cards.some(c=>c.open))g.cards[0].open=true;
  g.pending=null;g.flipped=null;
  ...
  else {g.turn++;rain(r,random);g.phase='choose';drawCandidates(g,random);}
}
function apply(room,uid,action,random,now=Date.now()){
  const r=structuredClone(room),me=r.players.indexOf(uid);
  if(me<0)fail('이 방의 참가자가 아닙니다.');normalizeCards(r.game);
  ...
  if(action.type==='ready'){
    ...
    const g=r.game;
    if(action.type==='flip'){
      // 뒤집힌 카드 1장을 뒤집어 본다. 한 차례에 한 장만, 뒤집은 카드는 이번 차례에 꼭 써야 한다.
      if(g.phase!=='choose')fail('지금은 카드를 뒤집을 수 없어요.');
      if(g.flipped)fail('이번 차례에는 카드를 한 장만 뒤집을 수 있어요.');
      const card=g.cards.find(c=>c.id===action.cardId);if(!card||card.open||card.flipped)fail('뒤집힌 카드를 골라 주세요.');
      card.flipped=true;g.flipped=card.id;
      ...
      if(g.flipped&&card.id!==g.flipped)fail('뒤집은 카드로 해야 해요.');
      if(!card.open&&!card.flipped)fail('뒤집힌 카드는 먼저 뒤집어 주세요.');
...
function view(r,uid){
  ...
  if(me>=0&&r.game){const {deck,rainTurns,...game}=structuredClone(r.game);normalizeCards(game);v.game=game;v.game.deckCount=deck.length;v.game.cards=game.cards.map(c=>c.open||c.flipped?{...c}:{id:c.id,open:false,flipped:false,hidden:true});v.actorIndex=actorIndex(r);if(game.pending)v.game.verdict=classify(game.board,game.pending.site,game.pending.k);}
  return v;
}
