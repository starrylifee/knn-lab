...
function view(r,uid){
  const me=r.players.indexOf(uid),v={id:r.id,version:r.version,status:r.status,seats:r.players.map(Boolean),ready:r.ready,leg:r.leg,totals:r.totals,me,winnerIndex:r.winnerIndex??null,finishReason:r.finishReason||null,firstIndex:r.firstIndex??null,redIndex:r.redIndex??null,seen:r.seen};
