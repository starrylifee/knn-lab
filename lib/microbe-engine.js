...
function view(r,uid){
  ...
  if(me>=0&&r.game){const {deck,...game}=r.game;v.game=structuredClone(game);v.game.deckCount=deck.length;v.game.cards=game.cards.map(c=>c.open||c.flipped?{...c}:{id:c.id,open:false,flipped:false,hidden:true});v.actorIndex=actorIndex(r);if(game.pending)v.game.verdict=classify(game.board,game.pending.site,game.pending.k);}
  return v;
}
