...
/* 뒤집힌 카드의 종류는 컴퓨터도 모른다(사람이 보는 view와 같은 정보만 쓴다). 뒤집은 카드가 있으면 그 카드만 쓸 수 있다. */
function decide(r,bi,random){
  const g=r.game,own=bi===r.redIndex?'red':'blue',turn=g.turn;
  const cards=E.view(r,BOT).game.cards.filter(c=>!g.flipped||c.id===g.flipped);
  const options=[];
  for(const site of g.candidates){
    ...
    for(const card of cards){
      if(card.hidden){options.push({site,cardId:card.id,flip:true,value:(p*base)-((1-p)*base)+.15+random(100)/400});continue;} // 모르는 카드: 기본 카드로 보고 약간의 기대치
...
function act(room,random,now=Date.now(),opts={}){const watch=opts.watchMs||WATCH_MS;
  ...
  const g=r.game;
  if(g.phase==='choose'){if(now-r.updatedAt<THINK_MS)return r;const {site,cardId,flip}=decide(r,bi,random);if(flip)return E.apply(r,BOT,{type:'flip',version:r.version,cardId},random,now);return E.apply(r,BOT,{type:'choose',version:r.version,site,cardId},random,now);}
  if(g.phase==='reveal'){
