...
function move(r,extra={}){return {type:'choose',version:r.version,site:r.game.candidates[0],cardId:r.game.cards[0].id,...extra};}
...
test('revealed choice is immutable and retry only works once with retry card',()=>{let r=start();const actor=r.players[E.actorIndex(r)];r.game.cards[0]={id:'test-retry',type:'retry'};const original=structuredClone(r);r=E.apply(r,actor,move(r),rng);assert.deepEqual(original.game.pending,null);assert.throws(()=>E.apply(r,actor,move(r),rng));r=E.apply(r,actor,{type:'reroll',version:r.version},()=>0);assert.equal(r.game.pending.k,1);assert.throws(()=>E.apply(r,actor,{type:'reroll',version:r.version},rng));r=E.apply(r,actor,{type:'confirm',version:r.version},rng);assert.equal(r.game.turn,2);assert.throws(()=>E.apply(r,actor,{type:'confirm',version:r.version},rng));});
test('ordinary cards cannot reroll; clients cannot supply K or score',()=>{let r=start();const uid=r.players[E.actorIndex(r)];r.game.cards[0]={id:'b',type:'basic'};r=E.apply(r,uid,move(r,{k:999,score:999}),()=>1);assert.equal(r.game.pending.k,3);assert.throws(()=>E.apply(r,uid,{type:'reroll',version:r.version},rng));assert.deepEqual(r.totals,[0,0]);});
